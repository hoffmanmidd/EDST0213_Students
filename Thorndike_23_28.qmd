...
```{r, echo = TRUE}
Table.2.1 = Table.2.1 %>% mutate(Gender = factor(Gender, levels=c("1", "2"), labels=c("male", "female")))
```

## Nominal Scale

From page 27 of Thorndike: "One way to define measurement is *the assignment of numbers to objects according to a set of rules*. The set of rules is called a scale. Knowiing the scale that has been used to assigned the numbers is critical to proper interpretation of the measurement. For example, Ms. Johnson and Mr. Cordero assigned the number 1 to their male students and the number 2 to their female students."
...
```{r, echo = TRUE}
Table.2.1 = Table.2.1 %>% mutate(Class = factor(Class, levels=c("1", "2"), labels=c("Johnson", "Cordero")))
```
...
```{r, echo = TRUE}
Table.2.1 <- Table.2.1 %>% mutate(Reading = as.integer(Reading), Spelling = as.integer(Spelling), Math = as.integer(Math))
```
